import { ResponseFormat, errorResponse, successResponse, sampleFormSchema } from '../../util/mSender';
import {
 ...
 * @returns {object} JSON
 */
const errorSender = (defaultError: ResponseFormat, customError?: any) => (customError ? { error: true, customError: (typeof customError === 'string' ? { message: customError, defaultError } : { customError, defaultError }) } : defaultError);
 ...
 * @returns {object} JSON
 */
const compareTwoObjectsKeys = (expectedRequest: any, userForm: any, allowNullOrUndefinedValue: boolean) => {
  let isNullOrUndefined = false;
  const keyWithIsNullOrUndefined: any[] = [];

  /*eslint-disable */
  if (!allowNullOrUndefinedValue && typeof allowNullOrUndefinedValue === 'boolean') {
  const validateDataType = (obj: any) => {
 ...
 * @returns {object} JSON
 */
  export const validateFields = (formType: string, formSchema: any, userForm: any, customError?: any, allowNullOrUndefinedValue?: boolean): ResponseFormat => {
  /**
   * @desc VALIDATE PARAMS
   */
  // first parameter
  if (typeof formType !== 'string') {
    return errorResponse('Unexpected Data-Type!', 400, 'argument', 'formType', 'Expected first argument should be string', { error: true, operationStatus: 'Processs Terminated!' });
  }
  // second parameter
  if (typeof formSchema === 'string' && !formSchema) {
    return errorResponse('Unexpected Data-Type!', 400, 'argument', 'formSchema', 'Expected second argument to be provided', { error: true, operationStatus: 'Processs Terminated!' });
  }
  if (formSchema === null || formSchema === undefined) {
    return errorResponse('Unexpected Data-Type!', 400, 'argument', 'formSchema', 'Second argument cannot be null or undefind', { error: true, operationStatus: 'Processs Terminated!' });
  }
  if (typeof formSchema !== 'object' || Array.isArray(formSchema)) {
    return errorResponse('Unexpected Data-Type!', 400, 'argument', 'formSchema', 'Expected second argument to be of an object: Your request schema should follow the format below', { error: true, operationStatus: 'Processs Terminated!', format: sampleFormSchema });
  }
  ...
  Object.keys(formSchema).forEach((request) => {
    if (typeof formSchema[request] !== 'object' || Array.isArray(formSchema[request]) || formSchema[request] == null) {
      x = true; xMsg = errorResponse('Unexpected Data-Type!', 400, 'argument', 'formSchema', 'Your request schema should follow the format below', { error: true, operationStatus: 'Processs Terminated!', format: sampleFormSchema });
      return;
    }
    if (typeof formSchema[request].formType !== 'string' || !formSchema[request].formType) {
      x = true; xMsg = errorResponse('Unexpected Data-Type!', 400, 'argument', 'formType missing in formSchema', 'Your request schema should follow the format below', { error: true, operationStatus: 'Processs Terminated!', format: sampleFormSchema });
      return;
    }
    ...
    if (!objects.length) {
      x = true; xMsg = errorResponse('Unexpected!', 400, 'fields-not-found', 'Form property', 'Pre-define your inputs. Follow the format below', { error: true, operationStatus: 'Processs Terminated!', format: sampleFormSchema });
      return;
    }

    objects.forEach((key) => {
      if (!(Object.keys(formSchema[request][key]).map((data) => data)).length) {
        x = true; xMsg = errorResponse('Unexpected Data!', 400, `${formType}.${key}`, `Schema.${formType}`, 'No Requirement found for input field', { error: true, operationStatus: 'Processs Terminated!', format: sampleFormSchema });
        return;
      }
      if (typeof formSchema[request][key].field !== 'string') {
        x = true; xMsg = errorResponse('Unexpected Data-Type!', 400, `${formType}.${key}`, `Schema.${formType}`, 'Schema object property "field" is missing or is not a string. Follow the format below', { error: true, operationStatus: 'Processs Terminated!', format: sampleFormSchema });
        return;
      }
      if (formSchema[request][key].field.trim() === '') {
        x = true; xMsg = errorResponse('Unexpected empty field!', 400, `${formType}.${key}`, `Schema.${formType}`, '"field" should be not be empty". Follow the format below', { error: true, operationStatus: 'Processs Terminated!', format: sampleFormSchema });
      }
    });
  });
  ...
  if (typeof userForm !== 'object' || Array.isArray(formSchema) || userForm === null) {
    return errorResponse('Expected Request-Body!', 400, 'argument', 'userForm', 'Expected third argument to be of an object: e.g { data: "my-data-value" }', { error: true, operationStatus: 'Processs Terminated!' });
  }

  // set variables
  const emailField: any[] = [];
  const phoneFields: any[] = [];
  const maxMinFields: any[] = [];
  const requiredFields: any[] = [];
  const expectedFields: any = {};
  const fieldsWithRange: any[] = [];
  const fieldsWithNameValues: any[] = [];
  const fieldsWithAlphaValues: any[] = [];
  const fieldsWithIntegerValues: any[] = [];
  const fieldsWithDecimalValues: any[] = [];
  const fieldsWithArrayAsValues: any[] = [];
  const fieldsWithObjectAsValues: any[] = [];
            ...
            Object.keys(userForm).forEach((key) => {
              if (obj.field === key) {
                if (isEmpty(userForm[key]) || !userForm[key]) {
                  const defaultError = errorResponse('ValidationError', 400, `${obj.field}`, `${formType}`, `${splitCamelCaseWord(obj.field)} cannot be empty`, { error: true, operationStatus: 'Processs Terminated!' });
                  errorMessage = errorSender(defaultError, customError);
                }
              }
            });
          });
            ...
            emailField.forEach((obj) => {
              if (userForm[key]) {
                if (key === obj.field && !isEmail(userForm[key])) {
                  const defaultError = errorResponse('ValidationError', 422, `${obj.field}`, `${formType}`, `${splitCamelCaseWord(obj.field)} is invalid. Email should look like e.g example@mail.com`, { error: true, operationStatus: 'Processs Terminated!' });
                  errorMessage = errorSender(defaultError, customError);
                }
              }
            });
            ...
            phoneFields.forEach((obj) => {
              if (userForm[key]) {
                if (obj.field === key && !isPhoneNumber(userForm[key])) {
                  const defaultError = errorResponse('ValidationError', 422, `${key}`, `${formType}`, `${splitCamelCaseWord(obj.field)} is invalid. You can try using a number like +2348180000009`, { error: true, operationStatus: 'Processs Terminated!' });
                  errorMessage = errorSender(defaultError, customError);
                }
              }
            });
              ...
              if (obj.field === key) {
                if (userForm[key]) {
                  if (userForm[key].length < obj.minLength || userForm[key].length > obj.maxLength) {
                    const defaultError = errorResponse('ValidationError', 400, `${key}`, `${formType}`, `The minLength characters expected for this field: ${splitCamelCaseWord(obj.field)}, is ${obj.minLength} with a maxLength of ${obj.maxLength}.`, { error: true, operationStatus: 'Processs Terminated!' });
                    errorMessage = errorSender(defaultError, customError);
                  }
                }
              }
            ...
            fieldsWithArrayAsValues.forEach((obj) => {
              if (userForm[key]) {
                if (obj.field === key && !Array.isArray(userForm[key])) {
                  const defaultError = errorResponse('ValidationError', 400, `${key}`, `${formType}`, `${splitCamelCaseWord(obj.field)} should be an array dataType`, { error: true, operationStatus: 'Processs Terminated!' });
                  errorMessage = errorSender(defaultError, customError);
                }
              }
            });
            ...
            fieldsWithObjectAsValues.forEach((obj) => {
              if (userForm[key]) {
                if (obj.field === key && typeof userForm[key] !== 'object' && userForm[key] !== null) {
                  const defaultError = errorResponse('ValidationError', 400, `${key}`, `${formType}`, `${splitCamelCaseWord(obj.field)} should be an object literal dataType`, { error: true, operationStatus: 'Processs Terminated!' });
                  errorMessage = errorSender(defaultError, customError);
                }
              }
            });
            ...
            fieldsWithDecimalValues.forEach((obj) => {
              if (userForm[key]) {
                if (obj.field === key && !isDecimal(userForm[key])) {
                  const defaultError = errorResponse('ValidationError', 400, `${key}`, `${formType}`, `${splitCamelCaseWord(obj.field)} should be a decimal value`, { error: true, operationStatus: 'Processs Terminated!' });
                  errorMessage = errorSender(defaultError, customError);
                }
              }
            });
            ...
            fieldsWithAlphaValues.forEach((obj) => {
              if (userForm[key]) {
                if (obj.field === key && !isAlpha(userForm[key])) {
                  const defaultError = errorResponse('ValidationError', 400, `${key}`, `${formType}`, `${splitCamelCaseWord(obj.field)} should only be alphabets`, { error: true, operationStatus: 'Processs Terminated!' });
                  errorMessage = errorSender(defaultError, customError);
                }
              }
            });
            ...
            fieldsWithIntegerValues.forEach((obj) => {
              if (userForm[key]) {
                if (obj.field === key && !isInteger(userForm[key])) {
                  const defaultError = errorResponse('ValidationError', 400, `${key}`, `${formType}`, `${splitCamelCaseWord(obj.field)} should only be integers`, { error: true, operationStatus: 'Processs Terminated!' });
                  errorMessage = errorSender(defaultError, customError);
                }
              }
            });
              ...
              if (obj.field === key) {
                if (userForm[key]) {
                  if (!isName(userForm[key])) {
                    const defaultError = errorResponse('ValidationError', 400, `${key}`, `${formType}`, `${splitCamelCaseWord(obj.field)} should be a proper name, and it should not contain special characters like: (#@$!±^&*+=">?{</}_|)`, { error: true, operationStatus: 'Processs Terminated!' });
                    errorMessage = errorSender(defaultError, customError);
                  }
                }
              }
              ...
              if (obj.field === key) {
                ...
                if (from === 'from' && to === 'to' && typeof obj.range[from] === 'number' && typeof obj.range[to] === 'number') {
                  if (typeof userForm[key] === 'number' && (userForm[key] < obj.range[from] || userForm[key] > obj.range[to])) {
                    const defaultError = errorResponse('ValidationError', 400, `${key}`, `${formType}`, `${splitCamelCaseWord(obj.field)} should be within range specification: ${obj.range[from]} - ${obj.range[to]}`, { error: true, operationStatus: 'Processs Terminated!' });
                    errorMessage = errorSender(defaultError, customError);
                  }
                }
              }
  ...
  return errorCompareKeys || errorMessage || successResponse('Success!', 200, 'validation', { error: false, operationStatus: 'Operation Completed', message: 'validation clean' });
};
